...
Con el raw_asset_id que ya usamos:
python -m scripts.import_overpass_places --raw-asset-id b805fd54-df1e-4326-b674-90ea4147ce2b

O apuntando directamente al directorio de deduplicación:
python -m scripts.import_overpass_places --dedup-dir "data/staging/osm_overpass/b805fd54-df1e-4326-b674-90ea4147ce2b/deduplication"

en un futuro lo actualzaré para hacer el flujo completo
'''

def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Ejecuta la importación canónica de candidatos deduplicados de Overpass "
            "hacia place, place_source_ref, place_category y place_neighborhood_assignment."
        )
    )
...
def main() -> int:
    ...
    logger.info(
        "Lanzando importador canónico Overpass | source_run_id=%s | raw_asset_id=%s | candidates=%s",
        source_run_id,
        raw_asset_id,
        len(candidates),
    )
    ...
    logger.info("Importación canónica Overpass completada correctamente.")
