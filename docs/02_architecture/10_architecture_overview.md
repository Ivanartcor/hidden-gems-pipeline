...
Su función no es solo descargar información desde varias fuentes, sino transformarla progresivamente hasta convertirla en una base canónica y una capa derivada de señales inteligentes capaz de alimentar análisis, ranking y futuras funcionalidades de producto.
...
La trazabilidad ya no se limita a la ingesta. También alcanza a la capa derivada de IA.
...
- `ai_model_version`, para registrar modelos y métodos como `dish_ner_transformer_v1`, `mention_sentiment_hybrid_v1_1` o `hidden_gems_ranking_v1`;
...
La arquitectura está planteada para soportar nuevas fuentes, nuevos modelos, nuevas reglas de ranking y una futura capa de API sin rediseñar el núcleo del sistema.
...
Este flujo se ha integrado inicialmente con Yelp Open Dataset como **prototipo IA**, no como ranking productivo de Sevilla.
...
- `scripts/` → puntos de entrada operativos, loaders y checks.
...
No representa todavía ranking final por barrios de Sevilla.
...
- exportación estándar de reviews desde PostgreSQL hacia flujos IA;
- piloto IA con reviews reales de Google Places Sevilla;
- adaptación multilingüe o española del detector de platos y sentimiento;
- ranking por barrio real (`ranking_scope = sevilla_neighborhood`);
- API mínima con FastAPI;
- dashboard o capa BI para explotación.
...
La arquitectura ya no solo prepara el terreno para NLP y ranking: actualmente contiene una integración IA completa validada con Yelp como prototipo, y queda lista para evolucionar hacia datos reales de Sevilla y ranking por barrio.
