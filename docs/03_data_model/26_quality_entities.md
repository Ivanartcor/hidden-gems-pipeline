...
## 12. Calidad y validación en Sevilla IA v2

La fase Sevilla IA v2 añade una capa de calidad adicional basada en evaluación de modelos, checks de artefactos y validación de dashboard.

### 12.1. Calidad de modelos entrenados

Modelos principales:

| Modelo | Tarea | Métrica destacada |
|---|---|---:|
| NER de platos v1.2 | detección de menciones | validación fuerte en dataset extendido |
| Normalization reranker v1 | entity linking de platos | accuracy@1 test = 1.0 |
| ABSA v1 | sentimiento por mención | macro F1 test = 0.9353 |

La calidad de los modelos se documenta en `docs/13_sevilla_ai_v2/` y se conserva mediante summaries de entrenamiento y predicción.

### 12.2. Calidad de normalización
...
| `low_confidence` | 58 |
| `no_candidate` | 27 |
| Ready for sentiment | 2.880 |

Las filas de baja confianza o sin candidato quedan separadas para revisión, no se ocultan.

### 12.3. Calidad de sentimiento ABSA
...
### 12.4. Calidad de señales place-dish
...
| Ready for ranking | 276 |
| Manual review rows | 2.075 |
| Unique places | 638 |
| Unique dishes | 182 |

La mayoría de señales son débiles o emergentes. Por eso el ranking v2 se presenta como experimental.

### 12.5. Calidad del ranking v2

Resultados seleccionados:

| Métrica | Valor |
|---|---:|
| Seleccionados v2 | 268 |
| Top hidden gems | 16 |
| Strong hidden gems | 77 |
| Promising hidden gems | 139 |
| Exploratory hidden gems | 36 |
| Production ready | 0 |

Tiers de evidencia:

| Evidence tier | Seleccionados |
|---|---:|
| emerging | 215 |
| solid | 48 |
| strong | 5 |
...
| Quality tier | Seleccionados |
|---|---:|
| high | 48 |
| medium | 199 |
| low | 21 |

### 12.6. Calidad del dashboard export

El export `dashboard_v2` valida:
...
- el ranking IA v2 tiene cero filas `production_ready`;
- v2 expande mucho respecto a v1 y debe explicar candidatos `v2_only`;
- hay candidatos `exploratory_hidden_gem` y deben existir filtros por tier/evidencia/calidad.

Estas advertencias no bloquean la entrega: forman parte de la honestidad técnica del prototipo.
