...
## 12. Estado actual y trazabilidad de Sevilla IA v2

Además del prototipo Yelp y del piloto Sevilla v1 cargado en PostgreSQL, la fase Sevilla IA v2 introduce una capa de trazabilidad basada en artefactos versionados y modelos locales.

Aunque no todos los resultados v2 se han persistido todavía en `ai_model_version` y `ai_pipeline_run`, la trazabilidad se conserva mediante:

- nombres de scripts versionados;
- carpetas de salida por fase;
- summaries JSON;
- contratos de datos;
- identificadores `PATCH_ID` en scripts críticos;
- modelos guardados localmente bajo `models/`;
- documentación específica en `docs/13_sevilla_ai_v2/`.

Flujo de trazabilidad v2:
...
Artefactos de control más relevantes:

| Capa | Summary / contrato |
|---|---|
| Hybrid + NER v2 | resumen de candidatos Hybrid + NER |
| Normalización | `sevilla_dish_normalization_summary_v1.json` |
| ABSA | `sevilla_absa_sentiment_summary_v1.json` |
| Señales | `sevilla_place_dish_signal_summary_v2.json` |
| Ranking | `sevilla_hidden_gems_ranking_v2_summary.json` |
| Comparación | `sevilla_ranking_v1_vs_v2_summary.json` |
| Dashboard export | `dashboard_export_summary.json`, `data_contract.json` |

Resultados finales trazados:
...
Para una futura fase de producción, el siguiente paso natural sería registrar formalmente estos modelos v2 y sus ejecuciones en `ai_model_version` y `ai_pipeline_run`, de forma equivalente al piloto v1 persistido.
