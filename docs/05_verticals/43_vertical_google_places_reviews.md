...
## 37. Actualización final: Google Places Reviews como base textual de Sevilla IA v2

La fase Sevilla IA v2 confirma que Google Places Reviews es una de las fuentes más importantes del proyecto, porque proporciona el texto local real sobre el que se construyen las señales gastronómicas.

La subvertical mantiene su responsabilidad original:

```text
adquirir reviews
→ guardar raw
→ transformar a staging
→ validar
→ importar en review
→ mantener enlace con place/place_source_ref/barrio
```

La IA se ejecuta después, como capa derivada e independiente.

### 37.1. Flujo final ejecutado

El flujo completo hasta Sevilla IA v2 quedó así:

```text
Google Places Reviews
→ hidden_gems.review
→ export reviews / candidatos para IA
...
Esto demuestra que la subvertical no solo está preparada para NLP, sino que ya ha alimentado una cadena IA completa.

### 37.2. Resultados de inferencia relacionados

La cadena Sevilla IA v2 generó los siguientes resultados a partir de menciones y reviews locales:

```text
Hybrid mentions = 2.979
NER mentions = 2.964
Unified candidate rows = 2.965
Normalized mention rows = 2.965
ABSA predicted rows = 2.880
Ready for downstream sentiment = 2.561
Place-dish signals = 2.335
Ready for ranking signals = 276
Selected ranking candidates v2 = 268
```

La distribución de sentimiento ABSA inferida fue:
...
### 37.3. Uso en el dashboard final

El export del dashboard v2 permite consultar ejemplos de menciones y reseñas mediante:
...
el dashboard puede mostrar el texto completo de reseña en la vista de detalle. Esto permite defender el ranking no solo con un score, sino con evidencia textual asociada.

### 37.4. Límites mantenidos

Aunque la subvertical ya ha permitido construir el ranking v2, se mantienen varias limitaciones:

- Google no entrega necesariamente todo el histórico de reviews;
- las reviews tienen sesgo positivo;
- algunas reseñas son breves o poco específicas;
- la evidencia puede ser parcial por local;
- el ranking sigue marcado como experimental;
- ninguna salida debe considerarse producción sin validación humana.

### 37.5. Estado final para entrega

```text
[OK] Reviews reales de Sevilla importadas
[OK] Reviews vinculadas a place/place_source_ref/barrio
[OK] Corpus local usado para piloto v1
[OK] Corpus local usado para Sevilla IA v2
[OK] Sentimiento ABSA aplicado por mención
[OK] Evidencia textual exportada para dashboard
[OK] Detalle de reseñas consultable en dashboard final
```

Con esto, Google Places Reviews queda cerrada como fuente textual local principal del proyecto integrado.
