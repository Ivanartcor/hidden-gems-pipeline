...
class PlaceCandidateImportResult:
    input_count: int
    imported_count: int
    skipped_count: int
...
class BasePlaceCandidateImporter:
    """
    Importador base para candidatos NormalizedPlaceCandidate hacia el modelo canónico.

    Responsabilidades comunes:
    - resolver source_system
    - buscar place_source_ref existente
    - matching básico contra place por nombre + distancia
    - crear/actualizar place
    - crear/actualizar place_source_ref
    - crear/actualizar categoría primaria
    - asignar barrio/distrito
    - registrar validation_issue
    - actualizar contadores de source_run

    Las verticales concretas solo deben configurar:
    - SOURCE_CODE
    - SOURCE_LABEL
    - notas específicas
    """
    ...
    SOURCE_CODE: str = ""
    SOURCE_LABEL: str = "Generic Source"
    SOURCE_ENTITY_TYPE_LABEL: str = "place_source_ref"

    CATEGORY_ASSIGNMENT_NOTE: str = "Asignación primaria derivada desde fuente externa."
    NEIGHBORHOOD_ASSIGNMENT_NOTE: str = "Asignación automática derivada desde fuente externa."

    def __init__(self) -> None:
        if not self.SOURCE_CODE:
            raise ValueError("El importador debe definir SOURCE_CODE.")
        self.logger = logger
        ...
        *,
        ...
        query = text(
            ...
            SET
                ...
                place_confidence = GREATEST(
                    COALESCE(place_confidence, 0),
                    COALESCE(:place_confidence, 0)
                ),
                is_active = TRUE
        ...
        if existing is None:
            connection.execute(
                ...
                ),
                {
                    ...
                    "category_id": category_id,
                    "source_system_id": source_system_id,
                    "notes": self.CATEGORY_ASSIGNMENT_NOTE,
                },
            )
        else:
            connection.execute(
        ...
        if row:
            return {
                ...
                "distance_to_centroid_m": (
                    float(row["distance_to_centroid_m"])
        ...
        if existing is None:
            connection.execute(
                ...
                ),
                {
                    ...
                    "notes": self.NEIGHBORHOOD_ASSIGNMENT_NOTE,
                },
            )
            return
    ...
    def import_candidates(
        ...
        *,
            ...
            for candidate in accepted_candidates:
                ...
                if (
                    ...
                    self._insert_validation_issue(
                        ...
                        source_run_id=source_run_id,
                        raw_asset_id=raw_asset_id,
                        entity_type=self.SOURCE_ENTITY_TYPE_LABEL,
                        entity_id=None,
        ...
        result = PlaceCandidateImportResult(
        ...
        self.logger.info(
            "Importador %s completado | imported=%s | skipped=%s | "
